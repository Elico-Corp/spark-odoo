...
class magento_product_category(orm.Model):
    ...
    def _get_parent_tree(self, parent_id):
        name = ""
        if parent_id:
            name = parent_id.name
            parent_name = self._get_parent_tree(parent_id.parent_id)
            if parent_name:
    ...
    def name_get(self, cr, uid, ids, context=None):
         ...
         for p in self.browse(cr, uid, ids, context=context):
             name = p.openerp_id.name
             if p.openerp_id.parent_id:
                 name = "%s / %s" % (
                    self._get_parent_tree(p.openerp_id.parent_id), name)
             if p.openerp_id.company_id:
                 name = "%s - %s" % (p.openerp_id.company_id.name, name)
