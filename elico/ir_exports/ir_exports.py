...
class ir_exports_line (osv.osv):
    ...
    def _get_filed_string(self, cr, uid, ids, fields_name, args=None, context=None):
        res    = {}
        f_pool = self.pool.get('ir.model.fields')
        
        for line in self.browse(cr, uid, ids, context=context):
            f_ids = f_pool.search(cr, uid, [ ('name','=',line.name),('model_id.model','=',line.export_id.resource)])
            f_id  = f_ids and f_ids[0] or False
            if f_id:
                res[line.id] = f_pool.browse(cr, uid, f_id).field_description
            else:
                res[line.id] = 'Null'
        return res
