...
def _create_magento_manufacturer_option(socket, session):
    """
        For one product manufacturer we need to create one attribute option
        in odoo , and then export the options to magento as attribute options
    """
    result = socket.read(
        session, 'product.manufacturer',
...
def export_all_product_scale(socket, session):
    ...
    scale_product_ids = socket.search(
        session, 'product.product', [('manufacturer_id', '!=', False)])

    socket.write(session, 'product.product', scale_product_ids, {})


def update_manufacturer_data(socket, session):
    """
        ...
        *   Update All the Old Data to bind new attribute
            and the backend
    """
    _model_name = 'product.manufacturer'
    ...
    attribute_ids = socket.search(
        ...
        [('attribute_code', '=', 'x_mmx_manufacturer')])
...
if __name__ == '__main__':
    ...
    update_manufacturer_data(socket, session)
    #Second We need to create one option for one scale data
    _create_magento_manufacturer_option(socket, session)
