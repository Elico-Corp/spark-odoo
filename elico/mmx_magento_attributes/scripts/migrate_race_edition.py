...
def _create_magento_race_edition_option(socket, session):
    """
        For one product race_editon we need to create one attribute option
...
def export_all_product_model(socket, session):
    """
        ...
        to magento
    """
    race_product_ids = socket.search(
        session, 'product.product', [('race_ed_id', '!=', False)])

    socket.write(session, 'product.product', race_product_ids, {})


def update_race_editon_data(socket, session):
    """
