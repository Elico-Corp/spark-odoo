...
class wizard_assign_shipment(osv.osv_memory):
    ...
    def _get_lines(self, cr, uid, context=None):
        ...
        for sol in sol_pool.browse(cr, uid, sol_ids):
            if sol.product_id.state == 'order':   #and not sol.sale_shipment_id
                data.append((0, 0, {
    ...
    _columns = {
        ...
        'shipment_id':fields.many2one('sale.shipment', "Shipment", required=True),
        'lines': fields.one2many('wizard.assign.shipment.line', 'wizard_id', 'SO Lines'),
    }
    _defaults = {
